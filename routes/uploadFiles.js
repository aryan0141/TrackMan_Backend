...
Router.get(
  "/deleteEveryClassv2/:courseName/:fileId",
  auth,
  async (req, res, next) => {
    try {
      ...
      mail.mailfunc(
        "Error in /deleteEveryClassv2/:courseName/:fileId",
        error.toString()
      );
      next(error);
    }
...
Router.get(
  "/deleteEveryClassSbv/:courseName/:fileId",
  auth,
  async (req, res, next) => {
    try {
      ...
      mail.mailfunc(
        "Error in /deleteEveryClassSbv/:courseName/:fileId",
        error.toString()
      );
      next(error);
    }
...
Router.post("/readForwardedMails", async (req, res, next) => {
  try {
    const data = req.body;
    const ReceivedSECRETKey = data.SECRETKEY;
    ...
    if (SECRET_KEY != ReceivedSECRETKey) {
      ...
      if (extension === "csv") {
        ...
        // Finding the existing class:
        var findClass = await completeClassv2.findOne({
          fileNames: { $elemMatch: { $eq: st3 } },
          teacher: teacherEmail,
        });
        if (findClass == -1 || findClass == null || findClass == undefined) {
          var class1 = {
            name: st3,
            teacher: teacherEmail,
            fileNames: [],
        ...
        completeClassv2
          ...
          .then((classFound11) => {
            ...
            let len1 = classFound11[0].uploadNames.length;
            for (let x = 0; x < len1; x++) {
              if (classFound11[0].uploadNames[x].filename === fileName) {
                console.log("File already exists");
                return res
                  .status(400)
                  .json({ status: 400, msg: "File already exists" });
              }
            }
            ...
            console.log(studentsData);
            const csvFilePath = `./public/ForwardedCSVFiles/${fileName}`;
            ...
            csv()
              ...
              .then(async (jsonObj) => {
                ...
                console.log(studentsData);
                await everyClassv2.create(studentsData);
                res.status(200).json({ msg: className1, status: 200 });
              });
          })
          .catch((error) => {
            console.log("Error in filename");
        ...
        const sbvFilePath = `./public/ForwardedCSVFiles/${fileName}`;
        var content = fs.readFileSync(sbvFilePath, "utf8").toString();
        const content1 = content.split("\n");
        ...
        if (findClass == -1 || findClass == null || findClass == undefined) {
          return res
                  .status(404)
                  .json({ status: 404, msg: "No class found for this SBV File" });
        }
        ...
        completeClassv2
          ...
          .then(async (classFound1) => {
            ...
            console.log("HeeJI", classFound1[0].name);

            let len1 = classFound1[0].uploadNames.length;
            for (let x = 0; x < len1; x++) {
              if (classFound1[0].uploadNames[x].filename === fileName) {
                console.log("File already exists");
                return res
                  .status(400)
                  .json({ status: 400, msg: "Error in FileName" });
            ...
            await everySBVv2.create(studentsData);
            res.status(200).json({ msg: classFound1[0].name, status: 200 });
          })
          .catch((error) => {
            console.log("Error in filename");
            return res
