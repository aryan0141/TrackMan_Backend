  ...
  try {
    ...
    if(SECRET_KEY != ReceivedSECRETKey) {
      ...
      if (extension === "csv") {
        ...
        var className1 = "";
        completeClassv2
          .find({ fileNames: { $elemMatch: { $eq: st3 } } , teacher: teacherEmail })
          .then((classFound11) => {
            // console.log(classFound11);
            className1 = classFound11[0].name;
            const thresholdMins1 = classFound11[0].cutOffMins;
            ...
            for( let x = 0; x<len1 ; x++){
              if(classFound11[0].uploadNames[x].filename === fileName){
                ...
                return res.status(400).json({ status: 400, msg: "Error in FileName" });
              };              
            }
        ...
        completeClassv2
          .find({ fileNames: { $elemMatch: { $eq: name4 } } , teacher: teacherEmail })
          .then(async (classFound1) => {
            var studentsData = {
              date: date1,
              className: classFound1[0].name,
