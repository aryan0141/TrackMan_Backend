...
Router.post("/updateDataForwardedMails", async (req, res, next) => {
  try {
    const data = req.body;
    ...
    const className2 = data.className;
        ...
        allCSV.forEach(async (classFound2) => {
          ...
          for (let x = 0; x < arrOfStudents.length; x++) {
            ...
            if (count === 0) {
              ...
              var zeroOrOne = 0;
              console.log(time3 , classFound.cutOffMins);
              if(time3>=classFound.cutOffMins){
                zeroOrOne = 1;
              }
    ...
    mail.mailfunc(
      "Error in /updateDataForwardedMails/:className/:teacher",
      error.toString()
    );
    next(error);
  }
